...
public class ConfigCorruptedScreen extends Screen {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < TEXT_BODY.size(); i++) {
            final Text text = TEXT_BODY.get(i);
            // TODO: previously was text.asString(), check if compatible
            if (text.getString().isEmpty()) {
                continue;
            }

            drawTextWithShadow(matrices, this.textRenderer, text, 32, 68 + (i * 12), 0xffffff);
        }
    }
}
